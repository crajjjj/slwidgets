...
; ----------------------------------------------------------------------------
; SL Widgets fork of iWant Status Bars 2.09 (DaemonPrime, MIT). Same
; public API, plus two accessors dependent mods use to mirror bar visibility
; and autohide state (_getBarVisible, _getBarLastChangeTime). "SL Widgets
; patch" comments mark the additions; the unmarked diffs vs stock 2.09 are
; bug fixes: bounded barsReady waits (stock spins forever if init never
; completes, and loadIcon did not wait at all), barsReady set before
; _claimBlankIcon (init deadlock), _findBarOfIcon comparing the icon id
; (stock matched any occupied slot), _nextFreeBarPosition guarded against
; bar == -1, alpha snap/redraw/transition ordering in _setIconStatusByID
; (per-tick pulse + hideOnAlpha0 positioning), initial widget alpha in
; _loadIconWidgets, and activation-key handlers redrawing only the bar
; whose key matched.
; ----------------------------------------------------------------------------
...
; SL Widgets patch: lets dependent mods detect this fork at runtime.
