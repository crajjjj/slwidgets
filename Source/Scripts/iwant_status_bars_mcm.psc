...
State BAR_X
	...
	Event OnSliderAcceptST(float value)
		...
		; SL Widgets patch: apply the new position immediately. Upstream
		; relied on the next icon-state change to redraw, so moving a bar
		; appeared to do nothing until some value happened to update.
...
State BAR_Y
	...
	Event OnSliderAcceptST(float value)
		...
		; SL Widgets patch: apply the new position immediately (see BAR_X).
		iBars._drawBar(activeBar)
...
Function _loadIcon(Int icon, FISSInterface fiss)
	; SL Widgets patch: released slots keep the "No Icon - No Icon"
